...
static void UHF_RFID_task(void *arg)
{
    ...
    while (1) 
    {
        ...
        /*********************************************************************************** */
            #if 01 //CL03_MODE
        if(xUHF_RFID_CenterParam.mode == SA_MODE || xUHF_RFID_CenterParam.mode == GROUP_MODE || xUHF_RFID_CenterParam.mode == SA_GROUP_MODE) 
        {
            //Nhận thông tin từ Center
            ...
            if (result > 0) { // 
                ...
                for(uint8_t i = old_MulPoll_recv_count; i < new_MulPoll_recv_count; i++) {
                    ...
                    if(x_UHFRFIDModeCL_2_Center_Queue != NULL) {
                        if(strncmp((char*)recv_cards[index].epc, "SA", 2) == 0) {
                            if(x_UHFRFIDModeCL_2_Center_Queue != NULL) {
                                // Bằng 0: Không thực hiện || Bằng 1: IN-Mode || Bằng 2: OUT-Mode || Bằng 3: SLEEP-Mode || Bằng 4: CL01-Mode ||bằng 5: CL03-Mode
                                xUHF_RFID_CenterParam.mode = SA_MODE;
                                // ESP_LOGI(TAG, "Check reset: %d", __LINE__);
                                xQueueSend(x_UHFRFIDModeCL_2_Center_Queue, &xUHF_RFID_CenterParam.mode, pdMS_TO_TICKS(20));
                            }   
                            continue;
                        }
                        else if(strncmp((char*)recv_cards[index].epc, "GR", 2) == 0) {
                            if(x_UHFRFIDModeCL_2_Center_Queue != NULL) {
                                // Bằng 0: Không thực hiện || Bằng 1: IN-Mode || Bằng 2: OUT-Mode || Bằng 3: SLEEP-Mode || Bằng 4: CL01-Mode ||bằng 5: CL03-Mode
                                xUHF_RFID_CenterParam.mode = GROUP_MODE;;
                                xQueueSend(x_UHFRFIDModeCL_2_Center_Queue, &xUHF_RFID_CenterParam.mode, pdMS_TO_TICKS(20));
                            }   
                            continue;
                        }
                        else if(strncmp((char*)recv_cards[index].epc, "SG", 2) == 0) {
                            if(x_UHFRFIDModeCL_2_Center_Queue != NULL) {
                                // Bằng 0: Không thực hiện || Bằng 1: IN-Mode || Bằng 2: OUT-Mode || Bằng 3: SLEEP-Mode || Bằng 4: CL01-Mode ||bằng 5: CL03-Mode
                                xUHF_RFID_CenterParam.mode = SA_GROUP_MODE;;
                                xQueueSend(x_UHFRFIDModeCL_2_Center_Queue, &xUHF_RFID_CenterParam.mode, pdMS_TO_TICKS(20));
                            }   
                            continue;
                        }
